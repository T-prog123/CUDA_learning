...
__global__ void MatrixMul_inner_product(const float* d_A, const float* d_B, float* d_C, const int M, const int N, const int P){
    ...
    const int tile_a_idx = get_mat_ind(threadIdx.y, threadIdx.x, shared_tile_dim); // width = shared_tile_dim
    const int tile_b_idx = get_mat_ind(threadIdx.y, threadIdx.x, block_dim_x);    // width = block_dim_x


    const int C_index_x = threadIdx.y + blockIdx.y * blockDim.y; // row of C
    const int C_index_y = threadIdx.x + blockIdx.x * blockDim.x; // column of C
    //DPRINTF( "B_index_y %d\n", B_index_y);
    ...
    while (n_tile < (N + shared_tile_dim - 1) / shared_tile_dim){
        int A_index_y = threadIdx.x + n_tile * blockDim.x;
        int A_index = get_mat_ind(C_index_x, A_index_y, N);
        int B_index_x = threadIdx.y + n_tile * blockDim.y;
        int B_index = get_mat_ind(B_index_x, C_index_y, P);

        // assining values to the tile (in shared memory) depending on if we are in bound of the real matrices or not
        if (C_index_x < M && A_index_y < N){
            tile_A[tile_a_idx] = d_A[A_index];
            if (blockIdx.x ==0 && blockIdx.y == 0){
                DPRINTF("Tile A at global indexes %d, %d was filled with value %f\n", C_index_x, A_index_y, tile_A[tile_a_idx]);
            }
        }
        else{
            tile_A[tile_a_idx] = 0;
        ...
        for (int k=0; k<shared_tile_dim; k++){
            thread_value += tile_A[get_mat_ind(threadIdx.y, k, shared_tile_dim)] * 
            tile_B[get_mat_ind(k, threadIdx.x, block_dim_x)];
        }
        __syncthreads();
        n_tile += 1;
    }
    if (C_index_x < M && C_index_y < P){
        d_C[get_mat_ind(C_index_x, C_index_y, P)] = thread_value;
    }
...
void d_matrix_mul(const float* h_A, const float* h_B, const int M, const int N, const int P){
    ...
    const int grid_dim_x = min((P + block_dim_x - 1) / block_dim_x, 10000);
    const int grid_dim_y = min((M + block_dim_y - 1) / block_dim_y, 10000);
